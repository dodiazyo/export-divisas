    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        @media print {
          ...
          .bg-slate-50, .bg-slate-100, .bg-green-50, .bg-blue-50, .bg-indigo-50 {
          ...
          <div className="p-10 print:p-0" id="shift-receipt-content">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-3">
                    ...
                    {totalDOPInjected > 0 && (
                      <div className="flex justify-between items-center text-indigo-700 font-medium">
                        ...
                        <span className="font-mono">+ RD$ {totalDOPInjected.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                    {(shift.externalSalesTotal || 0) > 0 && (
                      <div className="flex justify-between items-center text-green-700 font-medium">
                        <span>Ventas Externas:</span>
                        <span className="font-mono">+ RD$ {(shift.externalSalesTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                          ...
                          {shift.dopBreakdown && Object.entries(shift.dopBreakdown)
                            ...
                            .map(([denom, qty]) => (
                              <tr key={denom} className="hover:bg-slate-50">
                                <td className="p-2 font-bold text-slate-500">RD${denom}</td>
                                <td className="p-2 text-center text-slate-400">x</td>
                                <td className="p-2 font-black text-slate-900">{qty}</td>
                                <td className="p-2 text-right font-mono text-slate-400">RD$ {(parseInt(denom) * parseInt(qty)).toLocaleString()}</td>
                              </tr>
                            ))}
                        </tbody>
                      </table>
                          ...
                          {shift.usdBreakdown && Object.entries(shift.usdBreakdown)
                            ...
                            .map(([denom, qty]) => (
                              <tr key={denom} className="hover:bg-green-50">
                                ...
                                <td className="p-2 text-right font-mono text-green-700">${(parseInt(denom) * parseInt(qty)).toLocaleString()}</td>
                              </tr>
                            ))}
                        </tbody>
                      </table>
            ...
            {injections.length > 0 && (
              <section className="mb-10">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest border-l-4 border-indigo-600 pl-3 mb-4">Inyecciones de Capital</h3>
                <div className="border border-indigo-200 rounded-xl overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-indigo-50 border-b border-indigo-200">
                      <tr>
                        <th className="p-4 text-left text-indigo-600 uppercase text-xs font-bold font-mono">Hora</th>
                        <th className="p-4 text-left text-indigo-600 uppercase text-xs font-bold font-mono">Administrador</th>
                        <th className="p-4 text-center text-indigo-600 uppercase text-xs font-bold font-mono">Moneda</th>
                        <th className="p-4 text-right text-indigo-600 uppercase text-xs font-bold font-mono">Monto</th>
                        <th className="p-4 text-left text-indigo-600 uppercase text-xs font-bold font-mono">Nota</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-indigo-50">
                      {injections.map(inj => (
                        <tr key={inj.id} className="hover:bg-indigo-50/30">
                          <td className="p-4 text-slate-400">
                            {new Date(inj.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </td>
                          <td className="p-4 font-bold text-slate-800">{inj.adminName}</td>
                          <td className="p-4 text-center">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-black ${inj.currency === 'USD' ? 'bg-green-100 text-green-700' : 'bg-indigo-100 text-indigo-700'}`}>
                              {inj.currency}
                            </span>
                          </td>
                          <td className="p-4 text-right font-black text-indigo-900">
                            {inj.currency === 'DOP' ? 'RD$' : '$'} {inj.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                          <td className="p-4 text-slate-500 italic text-xs">{inj.note || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="bg-indigo-50 border-t-2 border-indigo-200">
                      <tr>
                        <td colSpan={3} className="p-4 font-black text-indigo-800 uppercase text-xs tracking-wider">Total Inyectado (DOP)</td>
                        <td className="p-4 text-right font-black text-indigo-900">
                          RD$ {totalDOPInjected.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </td>
                        <td></td>
                      </tr>
                    </tfoot>
            ...
            {externalSales.length > 0 && (
              <section className="mb-10">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest border-l-4 border-purple-600 pl-3 mb-4">
                  Ventas Externas
                  <span className="ml-2 text-purple-600 font-normal">
                    — Total: RD$ {(shift.externalSalesTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </span>
                </h3>
                  ...
                  <table className="w-full text-[11px]">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="p-2 text-left text-slate-500 uppercase font-mono">Cajero</th>
                        <th className="p-2 text-left text-slate-500 uppercase font-mono">Items</th>
                    ...
                    <tbody className="divide-y divide-slate-100 font-medium">
                      {externalSales.map(sale => (
                        <tr key={sale.id} className="hover:bg-slate-50">
                          ...
                          <td className="p-2 font-bold text-slate-800">{sale.cashier}</td>
                          <td className="p-2 text-slate-600">
                            {sale.items?.map(item => `${item.concept} (${item.qty}x)`).join(', ')}
                          </td>
                          <td className="p-2 text-right font-black text-slate-900">
                            RD$ {(sale.total || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="p-4 text-right text-slate-500 uppercase text-xs font-bold font-mono">Total Movido (DOP)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {Object.entries(shift.cashierActivity).map(([name, data]) => (
                        <tr key={name} className="hover:bg-slate-50">
                          ...
                          <td className="p-4 text-right font-black text-slate-900">
                            RD$ {data.totalDOP.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {exchangeTxs.length > 0 && (
              <section className="mb-10">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest border-l-4 border-slate-300 pl-3 mb-4">Bitácora de Compra de Divisas</h3>
                <div className="border border-slate-200 rounded-xl overflow-hidden">
                  <table className="w-full text-[11px]">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
